...
rawFileName = '/media/nrhagen/Drobo/OpediaVault/observation/in-situ/cruise/WOA_climatology/doc/woa_climatology_metadata.xlsx'
############################

dataset_metadata = pd.read_excel(rawFileName, sheet_name = 0)
vars_metadata = pd.read_excel(rawFileName,sheet_name = 1)


""" Strings """
DB='Opedia'
Table_Name = 'tblWOA_Climatology'
Dataset_Name = 'tblWOA_Climatology'#dataset_metadata.iloc[0]['dataset_short_name']
Dataset_Long_Name = dataset_metadata.iloc[0]['dataset_long_name']
Data_Source = dataset_metadata.iloc[0]['dataset_doi']
Distributor = 'https://www.nodc.noaa.gov/OC5/woa13/'
...
spatial_res_list = list(vars_metadata['var_spatial_res'])
temporal_res_list = list(vars_metadata['var_temporal_res'])

keyword_list = list(vars_metadata['var_keywords'])
comment_list = list(ip.NaNtoNone(vars_metadata['var_comment']))
Temporal_Coverage_Begin_list = ['NULL'] * len(vars_metadata)
Temporal_Coverage_End_list = ['NULL'] * len(vars_metadata)
...
Make_ID_list = ['1'] * len(vars_metadata)#obs
Sensor_ID_list = ['2'] * len(vars_metadata) # insity
Process_ID_list = ['2'] * len(vars_metadata) # Reprocessed
Study_Domain_ID_list = ['10'] * len(vars_metadata)
