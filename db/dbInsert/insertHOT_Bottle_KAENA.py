...
def makeBulkBottle_KAENA_HOT():
    path = cfgv.rep_hot_raw + 'bottle_kaena.csv'    
    prefix = 'bot_kaena_hot'
    missingValue = -9 
    df = pd.read_csv(path)   
    if ' ' in df.columns:
...
def bulkInsertBottle_KAENA_HOT(tableName):
    dataTitle = 'HOT_Bottle_KAENA'
    print('%s  Inserting Bulk %s into %s.' % (datetime.today(), dataTitle, tableName))
    try:
        bulkPath = ''
        bulkPath = makeBulkBottle_KAENA_HOT()
...
tableName = 'tblHOT_Bottle_KAENA'
bulkInsertBottle_KAENA_HOT(tableName)
