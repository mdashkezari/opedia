...
def makeBulkPF_HOT():
    path = cfgv.rep_hot_raw + 'particleFlux.csv'    
    prefix = 'pf_hot'
    missingValue = -9 
    df = pd.read_csv(path)   
    if ' ' in df.columns:
    ...
    formatDate_Columns(df, ['sdate', 'edate'])
...
tableName = 'tblHOT_ParticleFlux'
bulkInsertPF_HOT(tableName)
