...
def makeBulkBottle_WHOTS50_HOT():
    path = cfgv.rep_hot_raw + 'bottle_whots50.csv'    
    prefix = 'bot_whots50_hot'
    missingValue = -9 
    df = pd.read_csv(path)   
    if ' ' in df.columns:
...
def bulkInsertBottle_WHOTS50_HOT(tableName):
    dataTitle = 'HOT_Bottle_WHOTS50'
    print('%s  Inserting Bulk %s into %s.' % (datetime.today(), dataTitle, tableName))
    try:
        bulkPath = ''
        bulkPath = makeBulkBottle_WHOTS50_HOT()
...
tableName = 'tblHOT_Bottle_WHOTS50'
bulkInsertBottle_WHOTS50_HOT(tableName)
