...
def makeBulkBottle_ALOHA_HOT():
    path = cfgv.rep_hot_raw + 'bottle_aloha.csv'    
    prefix = 'bot_aloha_hot'
    missingValue = -9 
    df = pd.read_csv(path)   
    if ' ' in df.columns:
...
def bulkInsertBottle_ALOHA_HOT(tableName):
    dataTitle = 'HOT_Bottle_ALOHA'
    print('%s  Inserting Bulk %s into %s.' % (datetime.today(), dataTitle, tableName))
    try:
        bulkPath = ''
        bulkPath = makeBulkBottle_ALOHA_HOT()
...
bulkInsertBottle_ALOHA_HOT(tableName)
