...
def makeBulkBottle_KAHE_HOT():
    path = cfgv.rep_hot_raw + 'bottle_kahe.csv'    
    prefix = 'bot_kahe_hot'
    missingValue = -9 
    df = pd.read_csv(path)   
    if ' ' in df.columns:
...
def bulkInsertBottle_KAHE_HOT(tableName):
    dataTitle = 'HOT_Bottle_KAHE'
    print('%s  Inserting Bulk %s into %s.' % (datetime.today(), dataTitle, tableName))
    try:
        bulkPath = ''
        bulkPath = makeBulkBottle_KAHE_HOT()
...
tableName = 'tblHOT_Bottle_KAHE'
bulkInsertBottle_KAHE_HOT(tableName)
