...
function TileMODIS_NRT_CHL(itnums)
	addpath('tile/');
	Res = 0.01;
		...
		data=interpolate(LON,LAT,data',X,Y);
    		...
    		for i=1:rows
      			for j=1:cols
	        		...
	        		col=size(chl,2);
        			row=size(chl,1);
        			save(sprintf('/nfs/micklab004/mdehghani/opedia_vault/tile/obs/nrt/chl/nrt_chl_%10.10d_%d_%d.mat',itnum,i,j), 'chl', 'itnum', 'Res', 'col', 'row');
      			end
    		end
	end
end
...
function [path] = get_path(itnum)
	path = '/nfs/micklab004/mdehghani/opedia_vault/raw/obs/nrt/chl/nrt_chl_%d.nc';
