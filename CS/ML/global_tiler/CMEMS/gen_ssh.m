...
function gen_ssh(itnums,Xres,Yres)
  ...
  [lon lat ssh]=getSSH_CMEMS_dt(itnums(1));              %%%%   CHANGE TO NRT OR DT
  %[lon lat ssh]=getSSH_CMEMS_nrt(itnums(1));              %%%%   CHANGE TO NRT OR DT
...
%  lat=ceil(min_lat):Yres:fix(max_lat);
  ...
  for itnum=itnums
    ...
    [lon lat ssh]=getSSH_CMEMS_dt(itnum);                   %%%%   CHANGE TO NRT OR DT
%    [lon lat ssh]=getSSH_CMEMS_nrt(itnum);                   %%%%   CHANGE TO NRT OR DT
...
function [lon lat SSH]=getSSH_CMEMS_nrt(itnum)
path=strcat('/nobackup1/mdehghani/obs/CMEMS/ssh_nrt/nrt_sla_ssh_CMEMS_',sprintf('%7.7d',itnum),'.nc');
...
function [lon lat SSH]=getSSH_CMEMS_dt(itnum)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  AVISO  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
path=strcat('/nobackup1/mdehghani/obs/CMEMS/ssh/sla_ssh_CMEMS_',sprintf('%7.7d',itnum),'.nc');     %>>>>>>>>>  sea level anomaly
ncid=netcdf.open(path);
%[numdims,numvars,numglobalatts,unlimdimid] = netcdf.inq(ncid)   
%[name,xtype,dimids,natts] = netcdf.inqVar(ncid,3)
