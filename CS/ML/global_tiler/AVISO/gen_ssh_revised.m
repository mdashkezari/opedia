...
function gen_ssh_revised(itnums,Xres,Yres)
...
%    Y=ceil(min_lat):Yres:fix(max_lat);
    X=fix(min_lon):Xres:fix(max_lon);
    Y=fix(min_lat):Yres:fix(max_lat);
...
%  lat=ceil(min_lat):Yres:fix(max_lat);
  lon=fix(min_lon):Xres:fix(max_lon);
  lat=fix(min_lat):Yres:fix(max_lat);
  ...
  for itnum=itnums
    ...
    col=size(ssh,2);
    row=size(ssh,1);   
    %save(sprintf('/nobackup1/mdehghani/CS_Trunk/ML/ssh/ssh_%10.10d.mat',itnum), 'ssh', 'itnum', 'Xres', 'col', 'row', '-v7.3');
%    save(sprintf('/nobackup1/mdehghani/CS_Trunk/ML/ssh/ssh_%10.10d.mat',itnum), 'ssh', 'itnum', 'Xres', 'col', 'row');

    save(sprintf('/nobackup1/mdehghani/CS_Trunk/ML/ssh/ssh_%10.10d.mat',itnum), 'ssh', 'itnum', 'lon', 'lat');    
